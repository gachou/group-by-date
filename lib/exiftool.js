...
async function load (file, options) {
  ...
  const output = await runExifTool([
    '-G',
    '-json',
    '-struct',
    '-dateFormat',
...
async function save (file, tags) {
  ...
  // Exiftool only accepts the JSON form { "EXIF:DateTimeOriginal": ... }
  // but the ExifToolData is of the form { "EXIF": { "DateTimeOriginal": ... } }
  // convert here...
  Object.keys(tags).forEach((groupName) => {
    let groupObj = tags[groupName]
    Object.keys(groupObj).forEach((tagName) => {
      input[`${groupName}:${tagName}`] = groupObj[tagName]
    })
  })
...
async function repair (file) {
  return runExifTool(['-all=', '-tagsfromfile', '@', '-all:all', '-unsafe', '-icc_profile', file])
}
 ...
 */
async function creationDate (file) {
