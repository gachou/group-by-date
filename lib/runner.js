    ...
    crawl(sourceDir, async function (file) {
      ...
      if (file.stat.isFile() && path.extname(file.path).match(/jpg|mp4|avi|m2t|mpg/i)) {
        ...
        try {
          ...
          targetPath = await renameStrategy(file)
          ...
          return _options.dryRun
            ...
            : await moveFile(sourcePath, absTargetPath, {overwrite: false})
        } catch (e) {
          console.error('Error while handling ' + file.path + ' target: ' + targetPath, e)
        } finally {
          process.stdout.write(':')
